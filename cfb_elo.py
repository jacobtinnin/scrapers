# -*- coding: utf-8 -*-
"""
Created on Sun Nov 8 15:41:12 2020
...
for year in range(2010,2011,1):
    ...
    def getDfs():
        ...
        for school in school_set: 
            url = "https://www.sports-reference.com/cfb/schools/" + school + "/"+str(year)+"-schedule.html"
            page = urlopen(url).read()
            soup = BeautifulSoup(page,features="lxml")
            pre_df = dict()
            ...
            featuresWanted =  {'date_game','game_location','opp_name',
                               'game_result','points','opp_points'} #add more features here!!
            ...
            df["opp_name"]= df["opp_name"].apply(lambda row: (row.split(")\xa0")[-1]).rstrip())
